...
class XoApplicator:  public TransactionApplicator {
 public:
    XoApplicator(TransactionUPtr _txn, GlobalStateUPtr _state) :
        ...
        address_mapper(AddressMapperIF::Create(XO_NAMESPACE))
	{ };
    ...
    void Apply()
    {
		...
		if (xoTransactionPayload.GetAction() == "delete")
		{
			if ( !bStateLoaded )
			{
				throw InvalidTransaction(
		...
		if (xoTransactionPayload.GetAction() == "take")
		{
...
public:
    ...
    {
        AddressMapperUPtr
            addr(AddressMapperIF::Create(XO_NAMESPACE));
...
int main(int argc, char** argv) {
	...
	try {
		...
		TransactionProcessorUPtr processor(
			TransactionProcessorIF::Create(connectString));
